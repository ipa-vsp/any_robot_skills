...
class DetectColorNode : public BT::StatefulActionNode
{
  ...
  public:
    ...
    {
        return {
            BT::InputPort<double>("pick_z_offset", 0.02, "Offset for picking in z-axis."),
    ...
    BT::NodeStatus onRunning() override
    {
        ...
        for (int i = 0; i < required_poses_; i++)
        {
            ...
            RCLCPP_INFO(node_->get_logger(), "Sum_Transformation: %s",
                        geometry_msgs::msg::to_yaml(t_sum.transform).c_str());
        }
        ...
        // std::string newLine = std::to_string(t_sum.transform.translation.x) + "," +
        // std::to_string(t_sum.transform.translation.y) + "," + std::to_string(t_sum.transform.translation.z); file <<
        // newLine<<std::endl; file.close();

        RCLCPP_INFO(node_->get_logger(), "Sum_Transformation_AFTER: %s",
                    geometry_msgs::msg::to_yaml(t_sum.transform).c_str());

        tf2::doTransform(transformed_pose, final_pose, t_sum);

        pre_pose.pose.orientation = home_pose_.orientation;
