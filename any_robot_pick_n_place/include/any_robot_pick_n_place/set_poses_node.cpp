...
class SetPosesNode : public BT::SyncActionNode
{
    protected:
        ...
        // Parameters params_; Load the parameters at mangaer node and pass it to this node
    ...
    public:
        SetPosesNode(const std::string &name, const BT::NodeConfiguration &config)
        ...
        BT::NodeStatus tick() override
        {
            ...
            if (!found_ik)
            {
                RCLCPP_ERROR(node_->get_logger(), "Could not find transform from %s to %s", planning_base_frame_.c_str(), jmg->getLinkModelNames().back().c_str());
                return BT::NodeStatus::FAILURE;
            }
            ...
            {
                auto planning_scene_monitor = planning_scene_monitor::LockedPlanningSceneRO(moveit_cpp_->getPlanningSceneMonitor());
